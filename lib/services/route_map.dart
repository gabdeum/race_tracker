...
class RouteMap {
  ...
  RouteMap({required this.fileName});

  Future<List<Map<String,Object?>>> getXmlContent(BuildContext context) async {

    final String _xmlString = await DefaultAssetBundle.of(context).loadString("assets/$fileName.xml");
  ...
  Future<Database> createDb() async {
    ...
    Database _database = await openDatabase(_path, version: 1,
        onCreate: (Database db, int version) async {
          ...
          await db.execute(
              'CREATE TABLE $fileName (id INTEGER PRIMARY KEY, lat REAL, lon REAL, alt REAL, dist REAL)');
        });
  ...
  insertDb(List<Map<String,Object?>> listData, Database? db) async {
    ...
    if(db != null){
      ...
      listData.forEach((element) {
        _batch.insert(fileName, element);
      });
      await _batch.commit();
  ...
  Future<List<Map<String, Object?>>> queryDb(Database? db, double? lat, double? lon) async {
    ...
    if(db != null){
      ...
      Map<String, Object?> _pointO = (await db.rawQuery('SELECT id, lat, lon, alt, dist, MIN((${lat ?? 0.0} - lat)*(${lat ?? 0.0} - lat) + (${lon ?? 0.0} - lon)*(${lon ?? 0.0} - lon)) as min FROM $fileName'))[0];
      Map<String, Object?> _pointA = (await db.query(fileName, where: 'id = ${(_pointO['id'] as num)-1}'))[0];
      Map<String, Object?> _pointB = (await db.query(fileName, where: 'id = ${(_pointO['id'] as num)+1}'))[0];
