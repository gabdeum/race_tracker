...
class _HomeState extends State<Home> {
  ...
  RouteMap routeMap = RouteMap(fileName: 'Test_1');
  Database? database;
          ...
          body: Center(
            child: Column(
              ...
              children: [
                ...
                OutlinedButton(
                    onPressed: () {Navigator.pushNamed(context, '/record_map');},
                    ...
                    OutlinedButton(
                        onPressed: () async {
                          ...
                          locationSubscription = locationStream?.listen((event) async {
                            results = await routeMap.queryDb(database, event.latitude, event.longitude);
                            setState(() {});
                          });
                        },
                ...
                OutlinedButton(
                    onPressed: () async {
                      listData = await routeMap.getXmlContent(context);
                      database = await routeMap.createDb();
                      await routeMap.insertDb(listData, database);
                    },
